            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="modal-dialog">
                    ...
                    <div class="modal-body"> 
                        <select class="form-select" aria-label="Default select example" name="selectIdEstado" id="selectIdEstado">
                            <option selected>Seleccione un estado</option>
                            @foreach($estadosArray as $estados)
                                <option value="{{$estados['id']}}" name="idestado" id="idestado">{{$estados['estado']}}</option>
                            @endforeach
                        </select>
                        ...
                        <div class="input-group mb-3">
                            ...
                            <input type="text" class="form-control" aria-describedby="basic-addon1" name="municipio" id="municipio">
                        </div>
                    </div>
                ...
                <!-- FINALIZA MODAL -->
                    ...
                    <div class="datagrid">
                        <table>
                            ...
                            @foreach($municipiosArray as $municipios)
                                <tr>
                                    <td>{{$municipios['id']}}</td>
                                    <td>{{$municipios['municipio']}}</td>
                                    <td>{{$municipios['estado']['estado']}}</td>
                                    <td>
                                        <a href="{{ route('eliminarMunicipio', $municipios['id']) }}">Borrar</a>
                                    </td>
                                </tr>
                             @endforeach
...
<script type="text/javascript">
    jQuery(document).ready(function(){
        $('#selectIdEstado').on('change', function() {
        idestado = $(this).val();
    });

        jQuery('#guardar').click(function(e){
          console.log("aprete ajqx");
          var municipio = document.getElementById("municipio").value;

          
          url = 'guardMunicipio/'+municipio+'/'+idestado;
